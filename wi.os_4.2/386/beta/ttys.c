...
void get_one_sio(char *dev_name, WINDOW *win, int offset);
void Draw_watch_head(WINDOW *win);
...
void get_one_sio(char *tty_name, WINDOW *win, int offset)
{
...
char tty_name[12] = "/dev/tty";
	...
	draw_sio_display(win, (char *)tty_name);

	/*
		check for port closed.
	*/
	Set_Colour(win, Text_Alarm);
	if (!(sios[offset].t_state & (ISOPEN | WOPEN))) {
		mvwaddstr(win, 2, 11, "port closed     ");
		mvwaddstr(win, 3, 11, "port disabled           ");
	} else if (term_to_speed[sios[offset].t_state & CBAUD] == 0)
		mvwaddstr(win, 2, 11, "port disabled   ");
	else {
		Set_Colour(win, Normal);
		update_sio_display(win,(char *)tty_name, &sios[offset]);
	}
...
void watch_one_sio(char *tty_name)
{
...
register int counter;
	...
	*/
	strcpy(full_tty_name, "/dev/tty");
	strcat(full_tty_name, tty_name);

	/* Don't know why ?? but strcat should do this */
	full_tty_name[strlen(full_tty_name)] = '\0';

	if ((access(full_tty_name, R_OK)) != 0) {
		/*
		...
		*/
		...
		Draw_A_Box(win, BUTTON_BOX);
		Set_Colour(win, Text_Alarm);
		mvwprintw(win, 1, 2, "Sorry I can't access %s", full_tty_name);
		Set_Colour(win, Normal);
		wnoutrefresh(win);
		beep();
		any_key();
	...
	/*
	 *	Get the index into the array to speed things up.
	 */
	sios = (struct tty *) malloc(SIO_TTYS * sizeof(struct tty));
	...
	*/
	...
	for (offset = counter = 0; counter < SIO_TTYS; counter++) {
		if (strcmp(tty_num[counter], tty_name) MATCHES) {
			break;
			offset = counter;
		}
	}
	...
	/*
	 *	Open the window and start to work.
	 */
	...
	Draw_watch_head(win);
	draw_sio_display(win, full_tty_name);
...
*/
	...
	while (TRUE) {
		...
		get_one_sio(full_tty_name, win, offset);
		wnoutrefresh(win);
		an_option();
		doupdate();
...
void draw_sio_display(WINDOW *win, char *full_tty_name)
{

	Set_Colour(win, Colour_Banner);
	mvwprintw(win, 0, 2, " Detail on %s ", full_tty_name);
	Set_Colour(win, Normal);
