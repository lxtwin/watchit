...
static char  user_id[] =  "Dapix Technologies - Demo Version.";
	...
	/* open a tmp window */
	if (size_flag)
		win = newwin(14, 53, 24, 25);
	else
		win = newwin(14, 53, 7, 25);
	...
	mvwaddstr(win, 9, 2, "Watchit User");
	mvwaddstr(win, 10, 2, "Current Time");
	mvwaddstr(win, 12, 2, "Age of System");
...
#	endif
#endif
	mvwprintw(win, 9, 14, ": %.35s", user_id);
	mvwprintw(win, 10, 14, ": %.24s", ctime(&ktime));
	mvwaddstr(win, 12, 15, ":");
