...
#ifdef _ALPHA
	if (Doc_Flag)
		fprintf(stderr, "\f\n\n\n\nwatchit ALPHA version %s%c DOCUMENT REL.\n", WI_VERSION, WI_REV);
	else
		fprintf(stderr, "\f\n\n\n\nWATCHIT Development version %s%c NOT FOR RESALE.\n", WI_VERSION, WI_REV);
	fprintf(stderr, "Copyright(C) Dapix Technologies 1991-1994.\n");
	fprintf(stderr, "\n\n\n\n\nwatchit now entering the system..............");
#else
#	ifdef _BETA
	if (Doc_Flag)
		fprintf(stderr, "\f\n\n\n\nwatchit BETA version %s%c DOCUMENT REL.\n", WI_VERSION, WI_REV);
	else
		fprintf(stderr, "\f\n\n\n\nwatchit BETA version %s%c NOT FOR RESALE.\n", WI_VERSION, WI_REV);
	fprintf(stderr, "Copyright(C) Dapix Technologies 1991-1994.\n");
	fprintf(stderr, "\n\n\n\n\nwatchit now entering the system..............");
#	else
...
#	endif
	...
	*/
	if ((colour_flag = has_colors()) == 0)
		...
		/*
			Are we running from ODT ?
			we might have to change the colour
		*/
		if (getenv("DISPLAY") == NULL) {
			...
			init_pair(Colour_Magenta, COLOR_MAGENTA, COLOR_WHITE);
			init_pair(Red_Banner,     COLOR_WHITE,   COLOR_RED);
			init_pair(Colour_Ltcyan,  COLOR_CYAN,    COLOR_WHITE);
			init_pair(Colour_Ltblue,  COLOR_BLUE,    COLOR_WHITE);
		} else {
			/* I'm sorry I can't do any better than this for ODT */
			init_pair(Normal,         COLOR_CYAN,    COLOR_BLACK);
			init_pair(Colour_White,   COLOR_BLUE,    COLOR_BLACK);
			init_pair(State_Normal,   COLOR_GREEN,   COLOR_BLACK);
			init_pair(State_Warning,  COLOR_MAGENTA, COLOR_BLACK);
			init_pair(State_Alarm,    COLOR_RED,     COLOR_BLACK);
			init_pair(Colour_Bold,    COLOR_BLACK,   COLOR_WHITE);
			init_pair(Colour_Banner,  COLOR_WHITE,   COLOR_BLUE);
 ...
 */
	...
	Set_Colour(bottom_win, Colour_Brown);
	mvwaddstr(bottom_win, 1, 3, "Watchit reading device entries from /dev - ");
	Set_Colour(bottom_win, Text_Warning);
	mvwaddstr(bottom_win, 1, 47, "Please wait.......");
	Set_Colour(bottom_win, Normal);
	wrefresh(bottom_win);
