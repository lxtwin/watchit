...
#ifdef _ALPHA
		if (Doc_Flag)
		mvwprintw(top_win, 1, 2, " %02d:%02d         Watchit - The UNIX Performance Monitor.    %s    %02d/%02d/%02d " ,
				tim->tm_hour, tim->tm_min, WI_VERSION,
				tim->tm_mday, (tim->tm_mon + 1), tim->tm_year);
		else
		mvwprintw(top_win, 1, 2, " %02d:%02d        Watchit - ALPHA VERSION (NOT FOR RESALE)  %s      %02d/%02d/%02d ",
				tim->tm_hour, tim->tm_min, WI_VERSION,
				tim->tm_mday, (tim->tm_mon + 1), tim->tm_year);
#else
#	ifdef _BETA
		...
		mvwprintw(top_win, 1, 2, " %02d:%02d         Watchit - BETA VERSION (NOT FOR RESALE)  %s      %02d/%02d/%02d ",
				tim->tm_hour, tim->tm_min, WI_VERSION,
				tim->tm_mday, (tim->tm_mon + 1), tim->tm_year);
#	else
