...
#define max_val(x) ((sys_value(x) > max_sysinfo.x) ? max_sysinfo.x = sys_value(x), sys_value(x) : max_sysinfo.x)

#define minfo_value(x) ((minfo.x - last_minfo.x) / Sleep_Time)
#define max_nval(x) ((minfo_value(x) > max_minfo.x) ? max_minfo.x = minfo_value(x), minfo_value(x) : max_minfo.x)
...
enum states {
	...
	Colour_Blue, Colour_Cyan, Colour_Grey, Colour_Brown
};
